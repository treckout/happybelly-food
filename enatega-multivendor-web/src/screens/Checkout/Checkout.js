...
import { paypalCurrencies, PaystackCurrencies } from "../../utils/currencies";
...
function Checkout() {
  ...
  async function onCompleted(data) {
    ...
    } else if (paymentMethod.payment === "Paystack") {
      window.location = `${SERVER_URL}Paystack/create-checkout-session?id=${data.placeOrder.orderId}&platform=web`;
    }
  }
  ...
  function checkPaymentMethod(currency) {
    if (paymentMethod.payment === "Paystack") {
      return PaystackCurrencies.find((val) => val.currency === currency);
    }
