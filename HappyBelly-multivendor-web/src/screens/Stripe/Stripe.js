...
import PaystackCheckout from "react-Paystack-checkout";
import { myOrders, orderPaystack } from "../../apollo/server";
...
import { SERVER_URL, Paystack_PUBLIC_KEY } from "../../config/constants";
import UserContext from "../../context/User";
import Analytics from "../../utils/analytics";
import useStyles from "./styles";

const ORDER_Paystack = gql`
  ${orderPaystack}
`;
...
function Paystack() {
  ...
  const { data, loading, error } = useQuery(ORDER_Paystack, {
    variables: {
      id: id,
    },
  });
  ...
  useEffect(async () => {
    await Analytics.track(Analytics.events.NAVIGATE_TO_Paystack);
  }, []);
  async function handleResponse() {
    ...
    const order = result.data.orders.find(
      (item) => item.orderId === data.orderPaystack.orderId
    );
    setLoader(false);
  ...
  const onToken = (token) => {
    setLoader(true);
    fetch(`${SERVER_URL}Paystack/charge?id=${order.orderId}`, {
      method: "post",
      headers: {
      ...
      .then((result) => {
        if (result.redirect === "Paystack/success") {
          handleResponse();
        } else {
          showMessage({
            type: "error",
            message: "Paystack card error",
          });
        }
      })
      .catch((error) => {
  ...
  const order = data?.orderPaystack ?? undefined;
...
export default React.memo(Paystack);
